...
| 0006 | Comparable intensity label classification and v1 repetition cap | product | accepted | Parser owns `NON_COMPARABLE_LABELS` (`E`, `LR`, `REC`); all other labels require a Target Range in comparable mode. `parsePlan` always uses comparable mode. Repetition cap is 50. |
...
| 0008 | Run Comparison is a distinct concept on an extracted shared metric-delta primitive | product | accepted | Model Run Comparison (two arbitrary Runs, plan-independent) as its own engine concept built on a neutral `numericDelta` primitive; refactor Comparable-History Delta onto the same primitive, leave Prescription path untouched. `delta = comparand − baseline`, pace sign unflipped. |
