...
- requireTargetRanges dead at plan-load seam
- numerically comparable classification
- target range numeric invariants
- zero distance/duration
- huge repetition count guard, if you decide it blocks Reliable

#### Sub-issue #75: Preserve actionable prescription diagnostics through plan loading

Covers:

- nested repetition should report unsupported
- loader boundary drops diagnostic structure/context
- parser short-circuits on first error
- offset declared but never populated or should be removed

#### Sub-issue #76: Surface prescribed-run override failures in CLI

Covers:

- explicit --prescribed-run failure must be visible
- CLI regression test for override failure
- engine invariant test for duplicate dates in another week

#### Sub-issue #77: Fix comparable-history current FIT exclusion

Covers:

- uppercase .FIT exclusion mismatch
- test on case-insensitive/current-run behavior

#### Then close the parent issue
